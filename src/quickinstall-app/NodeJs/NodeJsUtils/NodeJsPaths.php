...
class NodeJsPaths {
	...
	private const CONFIG_DIR = 'private/nodeapp/config';
	...
	public function getConfigDir(string $domain, string $relativePath = null): string {
		$domainPath = $this->appcontext->getWebDomainPath($domain);
		if (empty($domainPath) || !is_dir($domainPath)) {
			throw new \Exception("Error finding domain folder ($domainPath)");
		}
		return Util::join_paths($domainPath, self::CONFIG_DIR, $relativePath);
	}

	public function getDomainConfigDir(string $domain, string $relativePath = null): string {
		return Util::join_paths($this->getConfigDir($domain), $relativePath);
	}
